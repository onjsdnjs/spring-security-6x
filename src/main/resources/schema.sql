...
CREATE TABLE ROLE_HIERARCHY_RELATIONSHIP (
                                             hierarchy_id BIGINT GENERATED BY DEFAULT AS IDENTITY, -- <<< 새로운 PK 컬럼 추가
                                             parent_role_id BIGINT NOT NULL,
                                             child_role_id BIGINT NOT NULL,
                                             PRIMARY KEY (hierarchy_id), -- <<< hierarchy_id를 단일 PK로 설정
                                             UNIQUE (parent_role_id, child_role_id), -- <<< 역할 관계의 유일성을 보장하기 위한 UNIQUE 제약 조건 추가
